...
export default function BDEDashboard() {
  ...
  const stats = {
    ...
    earnings: referrals.filter((r) => r.kyc?.status === 'approved').length * 100, // ₹100 per approval
  };
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          {Object.entries(stats).map(([key, value]) => (
            <div key={key} className="bg-[#1e293b] rounded-lg p-4 shadow">
              <h2 className="text-sm text-gray-400 capitalize">
                {key === 'earnings' ? 'Total Earnings' : `${key} referrals`}
              </h2>
